...
public class OrePopulator extends BlockPopulator {
						...
						if (d13 * d13 + d14 * d14 < 1.0D) {
							for (int i5 = m; i5 <= i2; i5++) {
								...
								if ((d13 * d13 + d14 * d14 + d15 * d15 < 1.0D)
										&& ((world.getBlockTypeIdAt(i3, i4, i5) == Material.STONE.getId()) || (world
												.getBlockTypeIdAt(i3, i4, i5) == Material.NETHERRACK.getId()))) {
	...
	public void populate(World world, Random random, Chunk chunk) {
		...
		if (world.getEnvironment() == Environment.NORMAL) {
			for (int i = 0; i < 10; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				int y = random.nextInt(128);

				createClump(world, Material.GRAVEL, 32, x, y, z, random);
			}
			for (int i = 0; i < 20; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				...
				createClump(world, Material.COAL_ORE, 16, x, y, z, random);
			}
			for (int i = 0; i < 20; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				...
				createClump(world, Material.IRON_ORE, 8, x, y, z, random);
			}
			for (int i = 0; i < 3; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				...
				createClump(world, Material.GOLD_ORE, 8, x, y, z, random);
			}
			for (int i = 0; i < 8; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				...
				createClump(world, Material.REDSTONE_ORE, 7, x, y, z, random);
			}
			/*
			 * for (int i = 0; i < 2; i++) { int x = worldChunkX +
			 * random.nextInt(16); int z = worldChunkZ + random.nextInt(16); int
			 * y = random.nextInt(25);
			 * 
			 * createClump(world, Material.DIAMOND_ORE, 7, x, y, z, random); }
			 */
			for (int i = 0; i < 2; i++) {
				int x = worldChunkX + random.nextInt(16);
				int z = worldChunkZ + random.nextInt(16);
				...
				createClump(world, Material.LAPIS_ORE, 6, x, y, z, random);
			}
			...
			for (int i = 0; i <= 5; i++) {
				...
				createClump(world, Material.QUARTZ_ORE, 20, x, y, z, random);
			}
		}
	}
}